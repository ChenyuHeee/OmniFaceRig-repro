...
if [ -w /etc/systemd/system ]; then
  ...
  echo '<redacted>' | sudo -S cp -f "$UNIT_SRC" "$UNIT_DEST"
fi
...
echo '<redacted>' | sudo -S systemctl enable webapp
echo '<redacted>' | sudo -S systemctl restart webapp

echo "==> 5/5 verify"
sleep 3
echo '<redacted>' | sudo -S systemctl status webapp --no-pager -l | head -15 || true
